...
(** [init_state game] represents the initial state of [game]. The initial state
has the combination of letters and the turns left in the game. *)
val init_state : Game.t -> t

(** [turns st] gives the turns left in game state [st]. *)
val turns: t  -> int

(** [create com game st] represents the state [st] updated with the word created 
by the player in [game] with command [com]. *)
val create: Command.word -> Game.t -> t -> t

(** [current_player st] represents the player whose turn is active in state [st]. *)
